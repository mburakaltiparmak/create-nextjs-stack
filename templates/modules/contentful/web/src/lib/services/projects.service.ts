...
export class ProjectService {
  private static contentType = CONTENT_TYPES.PROJECTS;
  ...
  static async getAll(preview?: boolean) {
    ...
    const fetcher = async () => {
      ...
      return flattenEntries(items).map((row) =>
        normalizeReference(row, "client_id", "clients"),
      );
    };
    ...
    try {
      return await withCache(
        ...
        { tags: [CACHE_TAGS.PROJECTS], revalidate: CACHE_TIMES.PROJECTS },
        isPreview,
      );
    } catch (error) {
  ...
  static async getBySlug(slug: string, preview?: boolean) {
    ...
    const fetcher = async () => {
      ...
      return normalizeReference(flattenEntry(items[0]), "client_id", "clients");
    };
